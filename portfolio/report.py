...
def fig_cumulative_nav(models_data, rec_spans):
    ...
    ax.set_ylabel("Net Asset Value (log scale, $1 = Jan 1980)")
    ax.set_title("Cumulative Net Returns — All Models (1980–2024)")
...
def fig_rolling_sharpe(models_data, rec_spans):
    ...
    ax.set_title(f"Rolling {ROLLING_WINDOW}-Month Net Sharpe Ratio (1980–2024)")
    ax.legend(loc="upper left")
    _date_axis(ax)
    plt.tight_layout()
...
def fig_drawdown(models_data, rec_spans):
    ...
    ax.set_title("Drawdown from Peak — All Models (1980–2024)")
...
def fig_turnover(models_data, rec_spans):
    ...
    ax.set_title("Portfolio Turnover — Rolling 12-Month Average (1980–2024)")
    ax.legend(loc="upper left")
    _date_axis(ax)
    plt.tight_layout()
...
def fig_calendar_returns(models_data):
    ...
    ax.set_title("Calendar Year Net Returns — All Models (1980–2024)")
    ax.legend(loc="upper left")
    plt.tight_layout()
